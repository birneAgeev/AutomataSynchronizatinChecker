...
extern void icdfaPretty(icdfa_t icdfa){
  ...
  printf("%d %d %d\n", ICDFA_NSTATES(icdfa), ICDFA_NSTATES(icdfa) * ICDFA_NSYMBOLS(icdfa), ICDFA_NSYMBOLS(icdfa));
  for (i = 0; i < size; i++) {
	  printf("%d %d %d\n", i / ICDFA_NSYMBOLS(icdfa), ICDFA_ELEM(i, icdfa), i % ICDFA_NSYMBOLS(icdfa));
  }
}
