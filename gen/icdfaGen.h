...
typedef struct icdfagen {
	struct icdfa* icdfa;	    /**< the current ICDFA */
	struct icdfa* last_icdfa;	    /**< the last possible ICDFA */
  ...
  /**/
	struct __mpz_struct ith[1]; 		    /* ith automaton */
	struct __mpz_struct total[1]; 		    /* maximum number of automata that can be generated */
                            /* for random generation */
	struct icdfa* rnd; 		    /* random automaton */
	struct __gmp_randstate_struct rstate[1];
	struct __mpz_struct (*grid)[1];
} *icdfagen_t;
...
//extern "C"{
//	extern icdfagen_t __declspec (dllexport) icdfaGenInit(int nstates, int nsymbols, int ac_only, int ibias, int random);
//}
...
extern struct icdfa* icdfaGenNextICDFA(icdfagen_t icdfaGen);
extern struct icdfa* icdfaGenNextICDFAe(icdfagen_t icdfaGen);
extern struct icdfa* icdfaGenRandom(icdfagen_t gen, int incomplete);
extern int icdfaGenIsLast(icdfagen_t icdfaGen);
extern void icdfaGenGetTotal(icdfagen_t gen, struct __mpz_struct total[1]);
...
extern void icdfaGenFromInteger(icdfagen_t gen, struct __mpz_struct m[1]);
extern void icdfaGenToInteger(icdfagen_t gen, struct __mpz_struct m[1]);
 ...
 */
INLINE_DECL struct icdfa* icdfaGenCurrent(icdfagen_t icdfaGen)
{
